...
url_custom        = 'https://drive.google.com/uc?id=10wXGUfj2tmu3wjDCF0W_SBbJQj6fMpwr' # Fran and Enrique's dataset
# url_ffhq        = 'https://drive.google.com/uc?id=18IuElDSejTzTmpxCEH6VGQVrLKBfGAzQ' # karras2019stylegan-ffhq-1024x1024.pkl
...
def draw_truncation_trick_video(png, Gs, w, h, seeds, psis):
    print(png)
    latents = np.stack(np.random.RandomState(seed).randn(Gs.input_shape[1]) for seed in seeds)
    dlatents = Gs.components.mapping.run(latents, None) # [seed, layer, component]
    dlatent_avg = Gs.get_var('dlatent_avg') # [component]
...
def main():
    ...
    #draw_uncurated_result_figure(os.path.join(config.result_dir, 'figure02-uncurated-ffhq.png'), load_Gs(url_custom), cx=0, cy=0, cw=256, ch=256, rows=3, lods=[0,1,2,2,3,3], seed=10)
    # draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing.png'), load_Gs(url_custom), w=1024, h=1024, src_seeds=[639,701,687,615,2268], dst_seeds=[888,829,1898,1733,1614,845], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,18)])
    #draw_style_mixing_figure(os.path.join(config.result_dir, 'figure03-style-mixing.png'), load_Gs(url_custom), w=256, h=256, src_seeds=[639,701], dst_seeds=[888,829], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,18)])
    #draw_style_mixing_figure(os.path.join(config.result_dir, 'figure-style-mixing.png'), load_Gs(url_custom), w=256, h=256, src_seeds=[639,701], dst_seeds=[888,829], style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,18)])
    #draw_noise_detail_figure(os.path.join(config.result_dir, 'figure04-noise-detail.png'), load_Gs(url_custom), w=256, h=256, num_samples=100, seeds=[1157,1012])
    #draw_noise_components_figure(os.path.join(config.result_dir, 'figure05-noise-components.png'), load_Gs(url_custom), w=256, h=256, seeds=[1967,1555], noise_ranges=[range(0, 18), range(0, 0), range(8, 18), range(0, 8)], flips=[1])
    #draw_truncation_trick_figure(os.path.join(config.result_dir, 'figure08-truncation-trick.png'), load_Gs(url_custom), w=256, h=256, seeds=[91,388], psis=[1, 0.7, 0.5, 0, -0.5, -1])
    draw_truncation_trick_figure(os.path.join(config.result_dir, 'figure08-truncation-trick.png'), load_Gs(url_custom), w=256, h=256, seeds=[91,388], psis=np.arange(-1, 1, 0.005))
    # draw_truncation_trick_figure(os.path.join(config.result_dir, 'figure08-truncation-trick.png'), load_Gs(url_custom), w=256, h=256, seeds=[32,64], psis=np.arange(-1, 1, 0.1))
